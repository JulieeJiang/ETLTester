...
test_mapping = mapping("L3_SRVR_PATCH_DTL_F") do
	...
	declare_target_table 'itr23.L3_SRVR_PATCH_DTL_F', 't'
	...
	m t.it_asset_asgn_grp_org_hier_ky do 
		...
		srvr_d.left_join(ci, 'ci.ci_lgcl_nm = srvr_d.srvr_ci_lgcl_nm').left_join(lkp_asset_org, 'lkp_asset_org.ASGN_GRP_ORG_HIER_ID = ci.IT_ASSET_OWN_ORG_HIER_ID')
	...
	m t.supp_own_asgn_grp_org_hier_ky do
		...
		srvr_d.left_join(lkp_supp_org, 'lkp_supp_org.ASGN_GRP_ORG_HIER_ID = ci.SUPP_OWN_ORG_HIER_ID')
		if srvr_d.srvr_ci_lgcl_nm.nil?
			-2 # not avail
		else
	...
	m t.bus_own_asgn_grp_org_hier_ky do
		...
		srvr_d.left_join(lkp_bus_org, 'lkp_bus_org.ASGN_GRP_ORG_HIER_ID = ci.SUPP_OWN_ORG_HIER_ID')
		if srvr_d.srvr_ci_lgcl_nm.nil?
			-2 # not avail
		else
	...
	m t.it_asset_own_pers_ky do
		...
		srvr_d.left_join lkp_asset_emp_ky, 'srvr_d.SRVR_CI_LGCL_NM = lkp_asset_emp_ky.ci_lgcl_nm'
		if srvr_d.srvr_ci_lgcl_nm.nil?
			-2 # not avail
		else
	...
	m t.supp_own_pers_ky do
		...
		srvr_d.left_join lkp_supp_emp_ky, 'srvr_d.SRVR_CI_LGCL_NM = lkp_supp_emp_ky.ci_lgcl_nm'
		if srvr_d.srvr_ci_lgcl_nm.nil?
			-2 # not avail
		else
	...
	m t.bus_own_pers_ky do
		...
		srvr_d.left_join lkp_bus_emp_ky, 'srvr_d.SRVR_CI_LGCL_NM = lkp_bus_emp_ky.ci_lgcl_nm'
		if srvr_d.srvr_ci_lgcl_nm.nil?
			-2 # not avail
		else
	...
	m t.srvr_ci_ky do
		...
		srvr_d.left_join ci_d, 'ci_d.ci_lgcl_nm = srvr_d.srvr_ci_lgcl_nm'
		if srvr_d.srvr_ci_lgcl_nm.nil?
			-2 # not avail
		else
			if ci_d.srvr_ci_ky.nil?
				0 # not found
			else
				ci_d.srvr_ci_ky
			end
